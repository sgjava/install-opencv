...
int test_trained_detector(String obj_det_filename, String test_dir,
	...
	for (size_t i = 0;; i++) {
		...
		for (size_t j = 0; j < detections.size(); j++) {
			Scalar color = Scalar(0, foundWeights[j] * foundWeights[j] * 200,
					0);
...
int main(int argc, char** argv) {
	const char* keys = { "{help h|     | show help message}"
	...
	if (train_twice) {
		...
		for (size_t i = 0; i < full_neg_lst.size(); i++) {
			...
			if (visualization) {
				for (size_t j = 0; j < detections.size(); j++) {
					rectangle(full_neg_lst[i], detections[j], Scalar(0, 255, 0),
							2);
				}
				imshow("testing trained detector on negative images",
						full_neg_lst[i]);
				waitKey(5);
			}
		}
